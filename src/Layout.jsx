...
import { LayoutDashboard, BookOpen, Target, BarChart3, Zap, Layers, Play, Upload, TrendingUp, Link as LinkIcon, Bot, MessageSquare, Shield, FileText, Menu, X, Wallet, Sun, Moon, Home, Users, User, Brain, GripVertical, Star, Clock, List, Bell, HelpCircle, UserCheck, Image as ImageIcon, Trophy, Filter, FileCheck, Plug } from 'lucide-react';
...
export default function Layout({ children, currentPageName }) {
  ...
  useEffect(() => {
    const checkOnboarding = async () => {
        try {
          ...
          // Skip onboarding check if already on an exempt page
          if (currentPageName === 'Landing' || currentPageName === 'Onboarding' || currentPageName === 'PlatformTour') {
            return;
          }
          ...
          // On error, don't redirect to onboarding — could be a transient network issue
          // Only redirect away if it's a genuine auth failure (no user)
          ...
          // Otherwise stay on the current page and let the user continue
        }
      };
  ...
  const defaultNavigation = [
    ...
    { id: 'bible', name: 'Trading Bible', page: 'TradingBible', icon: BookOpen },
    ...
    { id: 'connect', name: 'Connections', page: 'Connect', icon: Plug },
    ...
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      {!showAI && (
        <button
          ...
          className="fixed bottom-20 right-4 md:bottom-6 md:right-6 w-14 h-14 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full shadow-2xl hover:scale-110 transition-transform z-50 flex items-center justify-center group"
