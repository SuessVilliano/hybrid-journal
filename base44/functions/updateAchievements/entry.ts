 ...
 * Uses asServiceRole for all writes because the Achievement entity RLS
 * blocks backend updates that come from a different context than the owning user.
 */
Deno.serve(async (req) => {
  try {
    ...
    // Service role required — user-scoped writes are blocked by RLS on Achievement
    ...
    // 7-day streak
    if ((achievement.streak_days || 0) === 7 && !earnedBadgeIds.includes('week_warrior')) {
      const badge = await svc.entities.Badge.create({
        user_email: user.email,
    ...
    const badgeBonus = newBadges.reduce((sum, b) => sum + (b.points_awarded || 0), 0);
    const updated = await svc.entities.Achievement.update(achievement.id, {
      total_points: (achievement.total_points || 0) + points + badgeBonus,
      level: newLevel,
